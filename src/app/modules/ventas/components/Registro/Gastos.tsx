...
const Gastos = (props: GastosProps) => {
  ...
  const handleChangeValor = (
    ...
    updatedRows[index].valor = isNaN(value) ? 0 : value
    setRows(updatedRows)
  }
  ...
  const handleBlurValor = () => {
    const gastosDataNacionales = {
      nacionales: rows.filter((row) => row.campo !== '' && row.campo !== ''),
    }
    const gastosDataInternacionales = {
      internacionales: rowsInternacional.filter(
        (row) => row.campo !== '' && row.campo !== '',
      ),
    }

    if (gastosDataNacionales.nacionales.length === 0) {
      return
    } else {
      setValue('costosGastosNacionales', gastosDataNacionales.nacionales)
    }

    if (gastosDataInternacionales.internacionales.length === 0) {
      return
    } else {
      setValue('costosGastosInternacionales', gastosDataInternacionales.internacionales)
    }
  }
  ...
  // useEffect(() => {
  //   console.log('Se suponde que se resetea')
  //   resetComponent()
  // }, [isSubmitSuccessful, reset, isSubmitted])
    ...
    <Grid container spacing={2}>
      ...
      {rows.map((row, index) => (
        <Grid container item xs={12} spacing={2} key={index}>
            ...
            <Controller
              ...
              render={({ field }) => (
                <FormTextField
                  ...
                  id="costosGastosNacionales"
                  name="costosGastosNacionales"
                  value={row.valor.toLocaleString('es-ES')}
          ...
          {index === rows.length - 1 && (
            <Grid item xs={1}>
              <Fab color="primary" aria-label="add" onClick={handleAddRow} size="small">
                <AddIcon />
              </Fab>
            </Grid>
          )}
          ...
          {index === rowsInternacional.length - 1 && (
            <Grid item xs={1}>
              <Fab
                color="primary"
                aria-label="add"
                onClick={handleAddRowInternacional}
                size="small"
              >
                <AddIcon />
              </Fab>
